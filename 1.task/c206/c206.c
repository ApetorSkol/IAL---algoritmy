...
{
	//ZISTI CI JE LIST AKTIVNY
	if (list->firstElement == NULL)
	{
		return;
	}
	//POSTUUPNE SI VYTVORI DOCASNE PREMENNE POMOCOU KTORYCH BUDE PRECHADZAT
	//ZOZNAM A UVOLNOVAT PAMATA
	DLLElementPtr tmp = list->firstElement;
	DLLElementPtr tmpNext = tmp->nextElement;
	while (tmpNext != NULL)
	{
		free(tmp);
		tmp = tmpNext;
		tmpNext = tmpNext->nextElement;
	}
	free(tmp);
	// NASTAVI UKAZATELE LISTU NA NULL
	list->firstElement = NULL;
	list->lastElement = NULL;
	list->activeElement = NULL;
}
...
{
	// ALOKUJE PAMAT A SKONTROLUJE ZE CI BOLA SPRAVNE ALOKOVANA
	...
	// ALOKOVANE MIESTO DOSTANE SPRAVNE PARAMETRE
        tmp->data = data;
...
{
	// ALOKUJE PRIESTOR POTREBNY PRE PAMAT
        DLLElementPtr tmp = malloc(sizeof(struct DLLElement));
        if (tmp == NULL)
        {
                DLL_Error();
...
void DLL_DeleteAfter( DLList *list ) 
{
	...
	if (list->activeElement->nextElement->nextElement != NULL)
	{
		...
		list->activeElement->nextElement->previousElement = list->activeElement->nextElement->previousElement->previousElement;
	}
	else
	{
...
void DLL_DeleteBefore( DLList *list ) 
{
	...
	if (list->activeElement->previousElement->previousElement != NULL)
	{
		...
		list->activeElement->previousElement->nextElement = list->activeElement->previousElement->nextElement->nextElement;
	}
	else
	{
...
void DLL_InsertAfter( DLList *list, int data ) 
{
	...
	if (tmp != NULL)
	{
		tmp->previousElement = list->activeElement->nextElement;
	}
