...
/** pomocna funkcai ktora vracia prioritu mateamtickych operacii
*/
int Prio(int num)
{
        if (num == '+' || num == '-')
...
{
	char *Expr = malloc(sizeof(char)*MAX_LEN);
	...
	{
		if ((infixExpression[i] >= 48 && infixExpression[i] <= 57) || (infixExpression[i] >= 65 && infixExpression[i] <= 90) ||
			(infixExpression[i] >= 97 && infixExpression[i] <=122))
		{
			Expr[ExprLen] = infixExpression[i];
			ExprLen++;
		}
		else
		{
			doOperation(&stack,infixExpression[i],Expr,&ExprLen);
		}
		i++;
	}
	doOperation(&stack,'=',Expr,&ExprLen);
	Expr[ExprLen] = '=';
	return Expr;
}
